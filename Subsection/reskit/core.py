...
from numpy import mean, std, hstack, zeros, loadtxt
...
class Pipeliner(object):
    ...
    def __init__(self, steps, eval_cv=None, grid_cv=None, param_grid=dict(),
        ...
        self.test_fold = loadtxt('cv.txt')
        self.eval_cv = PredefinedSplit(test_fold = self.test_fold[0])
        self.grid_cv = PredefinedSplit(test_fold = self.test_fold[1])
    ...
    def get_scores(self, X, y, row_keys, scoring, collect_n=None):
        ...
        if not collect_n:
            ...
            for i in range(collect_n):
                eval_cv = PredefinedSplit(test_fold = self.test_fold[1+i])
